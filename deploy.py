...
def deploy_site():
    ...
    response.raise_for_status()
    site = next(
        (site for site in response.json()["sites"] if site["name"] == config["site_domain"]),None
    )
    ...
    if not site:
        ...
        nginx_template_id = next(
            (item["id"] for item in nginx_templates if item["name"] == config["nginx_template"]), None
        )
    ...
    if config["run_command"]:
        ...
        daemon_id = next(
            (
                ...
                if daemon["command"] == config["run_command"] and daemon["directory"] == f"/home/forge/{config["site_domain"]}"
            ),
            None,
        )
        if not daemon_id:
...
"""
    ...
    deployment = requests.get(
            f"{forge_uri}/servers/{server_id}/sites/{site_id}/deployment-history", headers=headers
        ).json()["deployments"][0]
